...
public record GraveResponseDTO(
                ...
                @Schema(description = "Indica se a sepultura está ativa.", example = "true")
                boolean active,

                @Schema(description = "Define o status da sepultura (AVAILABLE, OCCUPIED, MAINTENANCE).", example = "OCCUPIED")
                GraveStatus status,

                @Schema(description = "Informa se a sepultura está bloqueada.", example = "true")
                boolean blocked,

                @Schema(description = "Descreve a razão/motivo do bloqueio da sepultura.", example = "Processo jurídico aguardando resposta.")
                String reason,
